...
exports.deletedCompletedTask = (req, res) => {
    userTask.find({ customerId: req.body.deletedId }, (err, result) => {
        result.forEach(element => {
            if(element.state != 'rejected') {
                counter += 1
                if (counter == result.length) {
                    res.send({message: 'This offer is already taken by another user!'})
                }
            }else {
                res.send({message: 'Yes, we can restore this one!'})
            }
        });
    })
}
